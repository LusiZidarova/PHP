...
$lenghtStep = floatval(readline());
$distance = intval(readline());
$result = 0;

for($i=1; $i<=$steps; $i++){
    if($i%5 == 0){
        $result -= $lenghtStep*0.3;
    }
        $result += $lenghtStep;
}
$percentage  =number_format(($result/($distance*100))*100,2) ;
echo 'You travelled '.$percentage.'% of the distance!';
/*
Distance Calculator -Programming Fundamentals Mid Exam - 30 June 2019 Group 1 - Judge 100%
...
Then you have to calculate what distance you have travelled with the steps given. Have in mind that every fifth step is 30% shorter than usual.
