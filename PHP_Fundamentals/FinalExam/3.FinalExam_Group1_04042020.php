...
while($input != 'Sail'){
    ...
    $town  = $commands[0];
    $population = $commands[1];
    $gold = $commands[2];

    if(!key_exists($town,$cities)){
        $cities[$town] = array('population'=>$population,'gold'=>$gold);
    }else{
...
while($attaks != 'End'){
    ...
    switch ($args[0]){
        case 'Plunder':
            ...
            echo "$town plundered! $gold gold stolen, $people citizens killed.".PHP_EOL;

            if(($cities[$town]['population'] > 0 || $cities[$town]['population'] < $people) && ($cities[$town]['gold'] > 0 || $cities[$town]['gold'] < $gold)){
                $cities[$town]['population'] -= $people;
                $cities[$town]['gold'] -= $gold;

                if($cities[$town]['population'] ==0 || $cities[$town]['gold'] ==0){
                    echo "$town has been wiped off the map!".PHP_EOL;
                    unset($cities[$town]);
                }
            }
            break;
        case 'Prosper':
            ...
            if($gold < 0){
                echo "Gold added cannot be a negative number!".PHP_EOL;
            }else{
                $cities[$town]['gold'] += $gold;

                echo  "$gold gold added to the city treasury. $town now has ".$cities[$town]['gold']." gold.".PHP_EOL;
            }
    }
    $attaks = readline();
}

uksort($cities,function($v1,$v2) use ($cities) {

    if ($cities[$v1]['gold'] === $cities[$v2]['gold']) {
        return sort($cities);
    }
    return $cities[$v2]['gold'] <=> $cities[$v1]['gold'];
});

if(count($cities)>0){
    echo "Ahoy, Captain! There are ". count($cities)." wealthy settlements to go to:".PHP_EOL;
    foreach ($cities as $name => $town ){
        echo "$name -> Population: ".$town['population']." citizens, Gold: ".$town['gold']." kg".PHP_EOL;
    }
}else{
    echo "Ahoy, Captain! All targets have been plundered and destroyed!";
}


/*
Programming Fundamentals Final Exam 04.04.2020 - 100%
Problem 3. P!rates
...
"Prosper=>{town}=>{gold}"
oThere has been a dramatic economic growth in the given city, increasing its treasury by the given amount of gold.
...
oIf the given gold is a valid amount, increase the town's gold reserves by the respective amount and print the following message: "{gold added} gold added to the city treasury. {town} now has {total gold} gold."
Input
